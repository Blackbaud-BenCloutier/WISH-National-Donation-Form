(function ($) {
  ...
  mawf.donation_init = function (serverDefault) {
    ...
    if (mawf.hasSavedSession(mawf.cartName)) {
      ...
      if (mawf.bug && mawf.bug.sessionSaveActive) {
        console.log('Cart session cookie found');
      }
      ...
      if (mawf.bug && mawf.bug.sessionSaveActive) {
        console.log(mawf.cartName + ' deleted');
      }
    ...
    mawf.addFocusState = function (arg) {
      var $step;
      if (mawf.bug && mawf.bug.logStepFocusing) {
        console.log('+++ addFocusState() called by ' + arg.type + ' on ' + mawf.bug.show(arg.target), arg.target);
      }
      ...
      if ($step === undefined) { // Short ciruit or apply
        if (mawf.bug && mawf.bug.logStepFocusing) {
          console.log('addFocus error; Step not found');
        }
        return false;
    ...
    mawf.clearFocusState = function () {
      if (mawf.bug && mawf.bug.logStepFocusing) {
        console.log('--- clearFocusState() called');
      }
    ...
    $('.payment-types').on('focusin click', 'a', function (e) {
      mawf.addFocusState(e);
    });
  ...
  mawf.CartViewModel = function (sessionData) {
    ...
    mawf.initDates({
      date1: sessionData.startDate1,
      date2: sessionData.startDate2
    });
    ...
    if (($.querystring.CWSID || $.querystring.cwsid) && $.jStorage.index().length === 0) {
      if (mawf.bug && mawf.bug.logCWSIDImport === true) {
        console.log('Restore from CWSID triggered');
      }
      ...
      $.ajax({
        ...
        if (mawf.bug && mawf.bug.logCWSIDImport === true) {
          console.log('CWSID restore: returned', transaction);
        }
    ...
    self.getDateSuffix = function (intDate) {
      if ($.inArray(intDate, [1, 21, 31]) > -1) {
        return 'st';
      } // 1st 21st 31st
      else if ($.inArray(intDate, [2, 22]) > -1) {
        return 'nd';
      } // 2nd 22nd
      else if ($.inArray(intDate, [3, 23]) > -1) {
        return 'rd';
      } // 3rd 23rd
      else {
        return 'th';
    ...
    self.startDate1Pretty = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed startDate1Pretty fired');
      }
    ...
    self.startDate2Pretty = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed startDate2Pretty fired');
      }
    ...
    self.totalDonationToday = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed totalDonationToday fired');
      }
      var pretty, i, l,
        total = 0,
        d = self.donations();
    ...
    self.prettyTotalDonationToday = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed prettyTotalDonationToday fired');
      }
    ...
    self.totalDonationByDate1 = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed totalDonationByDate1 fired');
      }
      var total = 0,
        i, l, d = self.donations();
      for (i = 0, l = d.length; i < l; i++) {
    ...
    self.totalDonationByDate2 = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed totalDonationByDate2 fired');
      }
      var total = 0,
        i, l, d = self.donations();
      for (i = 0, l = d.length; i < l; i++) {
    ...
    self.allowAccess = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed allowAccess fired');
      }
    ...
    self.completeCardSelection = function () {
      ...
      $.scrollTo($('.js_donationConfigurator').last(), 300, {
        offset: -100
      });
    };
    ...
    self.resetProductBrowser = function () {
      if (mawf.bug.logPersonalizerSyncing) {
        console.log('resetProductBrowser() called');
      }
    ...
    self.getOpenDonation = function (returnType) {
      ...
      while (l--) {
        if (self.donations()[l].isInProcess()) {
          return returnType == 'int' ? l : self.donations()[l];
        }
      }
      return false;
    };
    ...
    self.syncRoot = function (syncList, syncDirection) {
      if (mawf.bug.logPersonalizerSyncing) {
        console.log('syncRoot(' + syncList + ', ' + syncDirection + ') called');
      }
      var l, to, list, from;
    ...
    self.clearPersonalizer = function (resetViews, donation) {
      if (mawf.bug.logPersonalizerSyncing) {
        console.log('clearPersonalizer(resetViews:' + resetViews + ' called on', donation);
      }
    ...
    self.sectionState = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed sectionState fired');
      }
      // Type section
      if (self.donationSelectInProcess()) {
        self.sectionTypeState({
          current: true
        });
      } else {
        self.sectionTypeState({
  ...
  mawf.CreateDonation = function (args, rootContext) {
    ...
    self.donationDest = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed donationDest fired');
      }
    ...
    self.honorName = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed honorName fired');
      }
    ...
    self.fullFromName = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed fullFromName fired');
      }
    ...
    self.prettyMemAttr = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed prettyMemAttr fired');
      }
    ...
    self.prettyProductFromAttr = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed prettyProductFromAttr fired');
      }
    ...
    self.fullToAddress = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed fullToAddress fired');
      }
      var fta;
    ...
    self.donationDate = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed donationDate fired');
      }
    ...
    self.setPrettyName = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed setPrettyName fired');
      }
      self.productPrettyName(self.productFirstName() + ' ' + self.productMI() + ' ' + self.productLastName());
      if (self.productPrettyName() == '  ') {
        self.productPrettyName('');
      } // Catch no names
    });
    ...
    self.prettyDonationAmount = ko.computed(function () {
      if (mawf.bug && mawf.bug.logComputeds === true) {
        console.log('computed prettyDonationAmount fired');
      }
  ...
  mawf.saveSession = function () {
    if (mawf.bug && mawf.bug.logSessionSaving) {
      console.log('----------------------------------\nsaveSession() called');
    }
    var prop,
      i,
    ...
    for (prop in state) { // Clean out sensitive data
      if (prop.indexOf('x_') === 0) {
        if (mawf.bug && mawf.bug.logSessionSaving) {
          console.log(prop + ' property removed.');
        }
        delete(state[prop]);
      }
    }
    ...
    for (i in state.donations) { // Remove donation amount, intent. and destnation data in incomplete donations
      ...
      for (prop in state.donations[i]) { // Clean out sensitive data
        if (prop.indexOf('x_') === 0) {
          if (mawf.bug && mawf.bug.logSessionSaving) {
            console.log(prop + ' property removed from donation ' + i + '.');
          }
      ...
      if (state.donations[i].isInProcess) {
        if (mawf.bug && mawf.bug.logSessionSaving) {
          console.log('Unsaved donation cleared.');
        }
  ...
  mawf.matchCategories = function (fromType, fromCat, toType) {
    ...
    if (fromType === toType) {
      console.warn('matchCategories(): fromType is the same as toType');
      return false;
    }
  ...
  mawf.handlingRequestToken = function (clearAnyExistingToken) {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('handlingRequestToken() called');
    }
    ...
    if (mawf.transToken === undefined || (mawf.transToken && clearAnyExistingToken)) {
        ...
        function (resp) { // Filter the response for errors returned as valid
          try { // Missing data or unexpected result code (at this 
            if (resp.CWSID === undefined || (resp.result !== 999 && jsonResp.resp !== 9999)) {
              if (mawf.bug && mawf.bug.logTokenTransaction) {
                console.warn('Missing or bad data in GetToken response', resp);
              }
            ...
            if (mawf.bug && mawf.bug.logTokenTransaction) {
              console.warn('json parse of GetToken response failed!');
            }
      ...
      chained.done(
        function (resp) {
          if (mawf.bug && mawf.bug.logTokenTransaction) {
            console.log('- Request token deferred resolved', resp.CWSID);
          }
      ...
      if (mawf.bug && mawf.bug.logTransactionChain) {
        console.log('- Shorting - token exists and reuse allowed called');
      }
  ...
  mawf.formatTransData = function (model) {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('formatTransData() called');
    }
    ...
    if (mawf.bug && mawf.bug.logTransDataBuild) {
      console.log('-----------------------------------------\n### Building Standard Transaction Data ###');
    }

    // DONATOR FIELDS - LOOP AND ASSIGN
    if (mawf.bug && mawf.bug.logTransDataBuild) {
      console.log('### Extracting Donator Field Template Values ###');
    }
    ...
    for (prop in donatorTemplate) {
      ...
      if (mawf.bug && mawf.bug.logTransDataBuild) {
        console.log('CW:' + prop + ' App:' + donatorTemplate[prop] + ' (' + modelVal + ')');
      }
    ...
    //
    if (model.updateInternational()) {
      postData.SendInfo = postData.EMail;
    } // National mailing list
    postData.MailingList = model.updateNational() ? "Yes" : "No"; // National mailing list
    if (model.updateLocal()) {
      postData.Custom1 = postData.EMail;
    } // Local mailing list
    ...
    for (i = 0; i < donLen; i++) {
      ...
      if (curDon.productAddress1() === '') {
        curDon.productCountry('');
      }

      // DONATION TEMPLATE    
      if (mawf.bug && mawf.bug.logTransDataBuild) {
        console.log('### Extracting Donation Field Template Values ###');
      }
      for (prop in donationTemplate) {
        modelVal = mawf.extractDonationTemplateValue(curDon, donationTemplate[prop]);

        if (mawf.bug && mawf.bug.logTransDataBuild) {
          console.log('CW:' + prop + '  ->  App:' + donationTemplate[prop] + '  "' + modelVal + '"');
        }
    ...
    if (mawf.bug && mawf.bug.logPostAsGet) {
      ...
      for (prop in postData) {
        str += prop + '=' + encodeURI(postData[prop]) + '&';
      }
  ...
  mawf.formatTransDataPP = function (model) {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('formatTransDataPP() called');
    }
  ...
  mawf.buildPayPalSubmitForm = function () {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('buildPayPalSubmitForm() called');
    }
  ...
  mawf.launchTransactionModal = function () {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('launchTransactionModal() called');
    }
  ...
  mawf.handlingEmailSubs = function () {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('handlingEmailSubs() called');
    }
    ...
    $.when(standardSignup, internationalSignup).then( // CHAIN EMAIL PROMISES
      function () { // Success - resolve
        if (mawf.bug && mawf.bug.logTransactionChain) {
          console.log('handlingEmailSubs() resolved');
        }
        return dfd.resolve();
      },
      function () { // Fail - but still resolve
        if (mawf.bug && mawf.bug.logTransactionChain) {
          console.log('handlingEmailSubs() failed');
        }
        return dfd.resolve();
      }
    );
  ...
  mawf.preparingFormData = function () {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('preparingFormData() called');
    }
    ...
    dfd.done(
      function (resp) {
        if (mawf.bug && mawf.bug.logTransactionChain) {
          console.log('preparingFormData() resolved');
        }
        mawf.transData = resp;
      }
    );
  ...
  mawf.handlingTokenAndData = function () {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('handlingTokenAndData() called');
    }
    var dfd = $.Deferred();
      ...
      .then(function () {
        $.when(mawf.preparingFormData())
          .then(
            function () {
              if (mawf.bug && mawf.bug.logTransactionChain) {
                console.log('handlingTokenAndData() resolved');
              }
              dfd.resolve();
            },
            function () {
              if (mawf.bug && mawf.bug.logTransactionChain) {
                console.log('handlingTokenAndData() failed');
              }
              dfd.reject();
            });
      });
  ...
  mawf.handlingFormSubmission = function () {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('handlingFormSubmission() called');
    }
    var dfd = $.Deferred();

    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('- Building multi promise ');
    }
    ...
    $.when(mawf.launchTransactionModal(), mawf.handlingTokenAndData(), mawf.handlingEmailSubs())
      .then(
        function () {
          if (mawf.bug && mawf.bug.logTransactionChain) {
            console.log('- Multi promise resolved');
          }
          dfd.resolve();
        });
    ...
    dfd.done(
      function () {
        if (mawf.bug && mawf.bug.logTransactionChain) {
          console.log('- Chained deferred firing');
        }
    ...
    dfd.fail(
      function () {
        if (mawf.bug && mawf.bug.logTransactionChain) {
          console.log('- Chained deferred failed');
        }
      }
    );
  };
  ...
  mawf.execTransaction = function () {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('execTransaction() called');
    }
    ...
    if (mawf.CartView.paymentType() == 'PayPal') {
      ...
      if (mawf.bug && mawf.bug.logTransactionChain) {
        console.log('transacting standard cc using endpoint ' + ajaxURL);
      }
        ...
        function (resp) { // Possible success
          ...
          try {
            ...
            if (resp.result === '0' || resp.result === '9999') {
              if (mawf.bug && mawf.bug.logTransactionChain) {
                console.log('Transaction filtering request response with ', resp);
              }
              return $.Deferred().resolve(resp);
            }
          } catch (e) {
            if (mawf.bug && mawf.bug.logTransactionChain) {
              console.log('Trans resp filter caught error with try/catch on parse of ', resp);
            }
          }
  ...
  mawf.getSuccessURL = function (appendToken) {
    if (mawf.bug && mawf.bug.logTransactionChain) {
      console.log('getSuccessURL() called');
    }
  ...
  mawf.reportError = function (data, sendGenericData) {
    if (mawf.bug && mawf.bug.logTransErrorHandling) {
      console.log('reportError() called with', data, sendGenericData, arguments);
    }
    ...
    if (mawf.bug && mawf.bug.logErrorReports) {
      console.log('Error report generated: ' + reportString);
    }
  ...
  mawf.collectGenericErrorData = function () {
    if (mawf.bug && mawf.bug.logTransErrorHandling) {
      console.log('collectGenericErrorData() called');
    }
  ...
  mawf.collectErrorData = function (template, code) {
    if (mawf.bug && mawf.bug.logTransErrorHandling) {
      console.log('collectErrorData() called');
    }
  ...
  mawf.packageErrorData = function (dataObj) {
    if (mawf.bug && mawf.bug.logTransErrorHandling) {
      console.log('packageErrorData() called');
    }
  ...
  mawf.processTransactionError = function (resp) {
    if (mawf.bug && mawf.bug.logTransErrorHandling) {
      console.log('processTransactionError() called with', resp);
    }
  ...
  mawf.getErrorGuide = function (serversCode) {
    ...
    try {
      codeInt = parseInt(serversCode, 10);
    } catch (e) {} // Get int (for mapping)
  ...
  mawf.handleError = function (guide, resp) {
    ...
    if (!mawf.transErrsLocalized) {
      mawf.integrateLocalizedTransErrorMessages();
    }

    if (guide.report) { // SEND ADMIN EMAIL
      mawf.reportError({
        template: guide,
        code: guide.convertedCode
      });
    }
    ...
    modalArgs.yes_cb = guide.modelAction ? function () {
      mawf.transactionErrorCB(guide.convertedCode);
      mawf.confirmModal.closeModal();
    } : function () {
      mawf.confirmModal.closeModal();
    };
  ...
  mawf.transactionErrorCB = function (code) {
    if (mawf.bug && mawf.bug.logTransErrorHandling) {
      console.log('transactionErrorCB() called');
    }
  ...
  mawf.donationSuccess = function () {
    ...
    window.setTimeout(function () {
      if (typeof mawf.bug === undefined || (mawf.bug && mawf.bug.blockSuccessRedirect !== true)) {
        window.location = mawf.getSuccessURL(true);
      }
    }, 1000);
  };
